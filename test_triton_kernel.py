#!/usr/bin/env python3
"""Compare DSpark Triton kernels against their Torch references on Ascend NPU.

Run from the SGLang repository root:

    python3 test_triton_kernel.py --device npu:0

Each case runs in a fresh subprocess by default.  This is intentional: a bad
Triton launch can poison the accelerator context and hide later results.  The
test deliberately bypasses every public ``execute`` dispatcher and calls the
operator's ``triton`` implementation directly, comparing it with the ``torch``
reference.  This makes unsupported NPU Triton paths fail visibly instead of
...
DEFAULT_MAX_ERROR_CHARS = 800
DEFAULT_MAX_TRACEBACK_CHARS = 0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Maximum characters retained for each error message.",
    )
    parser.add_argument(
        "--max-traceback-chars",
        type=int,
        default=DEFAULT_MAX_TRACEBACK_CHARS,
        help="Maximum characters retained for each traceback; 0 disables tracebacks.",
    )
    parser.add_argument(
        "--full-errors",
        action="store_true",
        help="Keep complete error messages and tracebacks (best used with --case).",
    )
    parser.add_argument(
        "--no-subprocess",
        action="store_true",
...
def compact_text(text: str, limit: int, *, full: bool = False) -> str:
    """Keep the useful beginning and root-cause tail of oversized diagnostics."""
    if full:
        return text
    if limit <= 0:
        return ""
...
def exception_fields(
    ...
    *,
    ...
    error = compact_text(
        f"{type(exc).__name__}: {exc}", max_error_chars, full=full_errors
    )
    trace = compact_text(
        traceback.format_exc(), max_traceback_chars, full=full_errors
    )
    return error, trace
...
def standard_case(
    *,
    ...
    max_traceback_chars: int = DEFAULT_MAX_TRACEBACK_CHARS,
    ...
    base = {
        ...
        "forced_triton": True,
        "execute_bypassed": True,
        "op_module": op.__module__,
    }

    triton_impl = getattr(op, "triton", None)
    if not callable(triton_impl):
        return {
            **base,
            "status": "TRITON_MISSING",
            "error": f"{op.__module__}.{op.__qualname__} has no callable triton()",
        }
    ...
    try:
        ...
        reference_error, reference_traceback = exception_fields(
            exc,
    ...
    try:
        torch.manual_seed(1234)
        actual = triton_impl(**clone_kwargs(kwargs))
        sync(torch, device)
    except Exception as exc:
        error, triton_traceback = exception_fields(
            exc,
            max_error_chars=max_error_chars,
            max_traceback_chars=max_traceback_chars,
            full_errors=full_errors,
        ...
        if reference_error is not None:
            error = compact_text(
                f"{error}; reference also failed: {reference_error}",
                max_error_chars,
                full=full_errors,
            )
        return {
...
def build_case(name: str, torch, device):
    ...
    if name == "commit_kv_proj":
        class TupleLinear(torch.nn.Module):
            def __init__(self):
                super().__init__()
                # Real SGLang linear modules always expose quant_method.  None
                # selects the plain BF16 stacked-weight path in this harness.
    ...
    if name == "build_ragged_verify_window":
        req_to_token = torch.arange(64, dtype=torch.int32, device=device).view(2, 32)
...
def emit_child_result(args: argparse.Namespace) -> int:
    try:
        result = run_one(
            args.case,
            args.device,
            max_error_chars=args.max_error_chars,
            max_traceback_chars=args.max_traceback_chars,
            full_errors=args.full_errors,
        )
    except Exception as exc:
        error, error_traceback = exception_fields(
            exc,
            max_error_chars=args.max_error_chars,
            max_traceback_chars=args.max_traceback_chars,
            full_errors=args.full_errors,
...
def run_parent(args: argparse.Namespace) -> int:
    ...
    for name in CASE_NAMES:
        cmd = [
            ...
            name,
            "--max-error-chars",
            str(args.max_error_chars),
            "--max-traceback-chars",
            str(args.max_traceback_chars),
        ]
        if args.full_errors:
            cmd.append("--full-errors")
        env = os.environ.copy()
        ...
        try:
            ...
            if marker is None:
                process_output = compact_text(
                    proc.stdout,
                    args.max_error_chars,
                    full=args.full_errors,
                )
                result = {
                    "case": name,
                    "status": "PROCESS_FAILED",
                    "error": f"exit={proc.returncode}; output={process_output}",
                }
            else:
                result = json.loads(marker[len("RESULT_JSON:") :])
        except subprocess.TimeoutExpired as exc:
            timeout_output = compact_text(
                exc.stdout or "",
                args.max_error_chars,
                full=args.full_errors,
            )
            result = {
                "case": name,
                "status": "TIMEOUT",
                "error": f"exceeded {args.timeout}s; output={timeout_output}",
            }
        results.append(result)
        display_error = compact_text(
            str(result.get("error", "")).replace("\n", " "),
            240,
            full=False,
        )
        print(
            f"{name:32s} {result['status']:24s} triton-direct "
            f"{display_error}"
        )
...
def run_in_process(args: argparse.Namespace) -> int:
    ...
    for name in CASE_NAMES:
        ...
        print(f"{name:32s} {result['status']:24s} {display_error}")
